...
;LoadSpritesTo Mario, $0200, #16
.macro LoadSpritesTo sprites, storeTo, spriteBytes
ldx #0
LoopSprite:
    lda sprites,x         ; We fetch bytes from the sprites lookup table
    sta storeTo,x              ; We store the bytes starting at OAM address
    inx                      ; X++
    cpx spriteBytes
    bne LoopSprite           ; Loop spriteBytes times (hardware sprites, 4 bytes each)
.endmacro
...
.macro Update4SpritesPosition xPos, yPos, oemAddress
    lda yPos
    sta oemAddress
...
UpdateSpritePosition:
    ...
    ;jsr UpdateMarioSprites
    Update4SpritesPosition XPos, YPos, $0200
    ;--------------------------------
    ; Mario:
    ;      Y   tile#  attributes   X
